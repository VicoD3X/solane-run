...
function Invoke-FrontendBrowserChecks {
  param([string]$RepoPath)

  $previousSmokeBaseUrl = $env:VITE_SMOKE_BASE_URL
  $env:VITE_SMOKE_BASE_URL = "http://127.0.0.1:4173"
  $preview = Start-Process `
    -FilePath "npm.cmd" `
    -ArgumentList @("--workspace", "apps/web", "run", "preview", "--", "--host", "127.0.0.1", "--port", "4173", "--strictPort") `
    -WorkingDirectory $RepoPath `
    -PassThru `
    -WindowStyle Hidden

  try {
    Invoke-External "npm.cmd" @("--workspace", "apps/web", "exec", "wait-on", "http://127.0.0.1:4173") $RepoPath
    Invoke-External "npm.cmd" @("run", "w3c:web") $RepoPath
    Invoke-External "node.exe" @("scripts\verify-ui.mjs") $RepoPath
  }
  finally {
    if ($preview -and -not $preview.HasExited) {
      Stop-Process -Id $preview.Id -Force
    }
    $env:VITE_SMOKE_BASE_URL = $previousSmokeBaseUrl
  }
}
...
try {
  ...
  if (-not $SkipChecks) {
    Invoke-Step "Frontend checks" {
      ...
      Invoke-FrontendBrowserChecks $webRepo
...
rollback() {
  ...
  if [ -d "$BASE/caddy" ]; then
    (cd "$BASE/caddy" && docker compose up -d --force-recreate) || true
  fi
}
